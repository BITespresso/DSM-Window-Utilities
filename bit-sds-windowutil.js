 ...
 */
{
    ...
    /**
     * Sets the window size and page XY position of the provided application. Currently open
     * application windows will be resized and moved to the provided page XY position.
     *
     * If a window is in `maximized` and/or `hidden` state, this method will not change this state.
     * However, after restoring the 'normal' state, the window will have the specified size and page
     * XY position.
     *
     * **Note**: If the provided width or height is smaller than the DSM internally defined
     * *minimum* width or height of the window and no window of the application is currently open,
     * then the width and height are set to the *default* width or height (instead of the *minimum*
     * width or height). This behaviour is due to an error in the DSM and cannot be changed.
     *
     * @param      {string}  appName  The application name.
     ...
     *
     * @example
     * BIT.SDS.WindowUtil.setSizeAndPagePosition("SYNO.SDS.App.FileStation3.Instance", 10, 49, 1600, 900);
     * // Sets the page XY position for File Station windows to (10px, 49px) and the size to 1600px x 900px
     */
    setSizeAndPagePosition: function(appName, x, y, width, height) {
        var restoreSizePosPropertyName = BIT.SDS.WindowUtil.getRestoreSizePosPropertyName(appName);
        ...
        if (appInstances.length > 0) {
            Ext.each(appInstances, function(appInstance) {
                ...
                if (appWindow.maximized || appWindow.hidden) {
                    elementPoints = BIT.SDS.WindowUtil.translatePagePositionToElementPoints(appWindow, x, y);
